...
import com.upg.xhh.mapping.borrower.FiUserRepayPlan;
...
public class HqlRemenber {
	...
	@Override
	public void updateRepayPlan(FiUserRepayPlan plan) {
		StringBuffer sb = new StringBuffer();
		sb.append("update FiUserRepayPlan set status=" + plan.getStatus() + ",mtime=now()" );
		if(plan.getActualRepayDate()!=null) {
			sb.append(",actual_repay_date='" + formatter.format(plan.getActualRepayDate()) + "'");
		}
		if(plan.getRestAmount()!=null){
			sb.append(",rest_amount=" + plan.getRestAmount());
		}	
		if(plan.getRepayedAmount()!=null){
			sb.append(",repayed_amount=" + plan.getRepayedAmount());
		}		
		if(plan.getRestPrinciple()!=null){
			sb.append(",rest_principal=" + plan.getRestPrinciple());
		}				
		if(plan.getRepayedPrinciple()!=null){
			sb.append(",repayed_principal=" + plan.getRepayedPrinciple());
		}				
		if(plan.getRestInterest()!=null){
			sb.append(",rest_interest=" + plan.getRestInterest());
		}				
		if(plan.getRepayedInterest()!=null){
			sb.append(",repayed_interest=" + plan.getRepayedInterest());
		}
		if(plan.getRestCommissionCharge()!=null){
			sb.append(",rest_commission_charge=" + plan.getRestCommissionCharge());
		}				
		if(plan.getRepayedCommissionCharge()!=null){
			sb.append(",repayed_commission_charge=" + plan.getRepayedCommissionCharge());
		}		
		if(plan.getRestPenaltyInterestAmount()!=null){
			sb.append(",rest_penalty_interest_amount=" + plan.getRestPenaltyInterestAmount());
		}				
		if(plan.getRepayedPenaltyInterestAmount()!=null){
			sb.append(",repayed_penalty_interest_amount=" + plan.getRepayedPenaltyInterestAmount());
		}			
		String hql = sb.append(" where period_number=" + plan.getPeriodNumber()).append(" and repay_info_id=" + plan.getRepayInfoId()).toString();
		batchUpdate(hql, null);								
	}
